...
echo [INFO] Extracting .docx documents to transcripts using extract_docx.py...
python.exe .\scripts\extract_docx.py
if %ERRORLEVEL% NEQ 0 (
    echo.
    echo [ERROR] Failed to extract transcripts from .docx files!
    echo.
    pause
    exit /b 1
)
...
python.exe .\scripts\generate_transcripts_json.py .\
...
python.exe .\scripts\index_chromadb_json.py .\
